...
function! s:command_for_url(params, action) abort
  ...
  for [k, v] in items(a:params)
    let op = '--'.k.'='.v
    if index(g:gcloud_global_ops, k) >= 0
      call add(cmd, op)
    else
      call add(subcmd, op)
    endif
...
function! db#adapter#cloudspanner#interactive(url, action) abort
...
function! db#adapter#cloudspanner#input(url, in) abort
  ...
  if len(matchstr(a:in, '.sql'))
    let out += db#adapter#cloudspanner#interactive(a:url, 'query')
